...
architecture rtl of fpga is
	...
	--CONSTANT DIG_MAX_SAMPLES : integer := 4096;
	...
	component dig_pattern_gen is
		...
		Port (
			...
			digitalPatternCompleteCnt : out STD_LOGIC_VECTOR (31 downto 0));
	end component;
	...
	signal digitalPatternCompleteCnt : STD_LOGIC_VECTOR (31 downto 0);
	signal digitalPatternCompleteCnt_d : STD_LOGIC_VECTOR (31 downto 0);
	signal digitalPatternCompleteCnt_dd : STD_LOGIC_VECTOR (31 downto 0);
	...
	attribute KEEP of digitalPatternCompleteCnt_d: signal is true;
	attribute ASYNC_REG of digitalPatternCompleteCnt_d: signal is true;
	attribute KEEP of digitalPatternCompleteCnt_dd: signal is true;
	attribute ASYNC_REG of digitalPatternCompleteCnt_dd: signal is true;
...
begin
	...
	dig_pattern_gen_0: dig_pattern_gen
		...
		port map (
			...
			digitalPatternOutputRestart => digitalPatternOutputRestart_dd,
			digitalClkDivide => std_logic_vector(digitalClkDivide),
			data_in => doutb_dig,
			data_out => doutb_dig_d,
			addrb_dig => addrb_dig,
			digitalPatternCompleteCnt => digitalPatternCompleteCnt
		);
	...
	begin
		...
		if (rising_edge(clk_adc_dclk)) then
			...
			-- digital pattern sequence restart conditions
			buffersel_d <= buffersel;
			buffersel_dd <= buffersel_d;
			digitalPatternOutputRestart_d <= digitalPatternOutputRestart;
			-- auto restart digital pattern on upload, if enabled
			if (buffersel_dd = "11" and buffersel_d = "00") and digitalPatternOutputRestartOnUpload = '1' then
				digitalPatternOutputRestart_dd <= '1';
			-- elsif restart command was received
			elsif (reading_config_registers_dd = '1' and reading_config_registers_d = '0') and digitalPatternOutputRestart_d = '1' then
				digitalPatternOutputRestart_dd <= '1';
			else
				digitalPatternOutputRestart_dd <= '0';
			end if;
			...
			case to_integer(unsigned(cfg_addrB_d) + 1) is
				...
				when 25 =>
					...
					digitalPatternOutputRestart <= cfg_do_B(26);
	...
	begin
		...
		if (rising_edge(ifclk)) then
			...
			digitalPatternCompleteCnt_d <= digitalPatternCompleteCnt;
			digitalPatternCompleteCnt_dd <= digitalPatternCompleteCnt_d;
					...
					-- send data to FX3 if EP6 is ready
						...
						if hword_cnt_i < FRAME_HEADER_SIZE then
							...
							case hword_cnt_i is
								...
								when 3 =>
									fdata <= std_logic_vector(digitalPatternCompleteCnt_dd);
